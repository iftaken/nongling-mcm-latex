% \documentclass{main}
\documentclass[withoutpreface,bwprint]{main} %去掉封面与编号页，电子版提交的时候使用。


\usepackage[framemethod=TikZ]{mdframed}
\usepackage{url}   % 网页链接
\usepackage{subcaption} % 子标题
\title{农林杯高校数学建模竞赛 \LaTeX{} 模板}
% \tihao{A}
% \baominghao{xxxx}
% \schoolname{XX大学}
% \membera{ }
...
\begin{document}
 ...
 \maketitle
 \begin{abstract}
本模板是为农林杯高校数学建模竞赛编写的 \LaTeX{} 模板，旨在让大家专注于论文的内容写作，而不用花费过多精力在格式的定制和调整上。本手册是相应的参考，其中提供了一些环境和命令可以让模板的使用更为方便。同时需要注意，使用者需要有一定的 \LaTeX{} 的使用经验，至少要会使用常用宏包的一些功能，比如参考文献、数学公式、图片使用、列表环境等等。
...
\begin{tcode}
    \documentclass{main}
    %\documentclass[withoutpreface,bwprint]{main} %去掉封面与编号页
...
\begin{figure}
    ...
    \begin{minipage}[c]{0.48\textwidth}
        \centering
        \includegraphics[height=0.2\textheight]{example}
        \subcaption{一只猫}
    \end{minipage}
    \begin{minipage}[c]{0.48\textwidth}
        \centering
